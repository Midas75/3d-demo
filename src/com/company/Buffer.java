...
public class Buffer {
    ...
    public TransformedTri[] transformedTriSet;
    public BUnit[][] bUnitSet;
        ...
        for(int i=minX;i<maxX;i++){
            for(int j=minY;j<maxY;j++){
                ...
                if((camera.obMode&Camera.SHOW_TEXTURE)!=0){
                    ...
                    int[] b=new int[]{(tv[0].color>>0)&0xFF,(tv[1].color>>0)&0xFF,(tv[2].color>>0)&0xFF};
    ...
    public void DDALine(int x0, int y0,int c0, int x1, int y1,int c1) {
        DDALine(x0,y0,(c0>>16)&0xff,(c0>>8)&0xff,(c0>>0)&0xff,x1,y1,(c1>>16)&0xff,(c1>>8)&0xff,(c1>>0)&0xff);
    }
    public void setBuffer(Camera camera) {
        Matrix.doWorldProjectTransform(this, camera);
        if ((camera.obMode & Camera.SHOW_LINE) != 0) {
            for (int i = 0; i < transformedTriSet.length; i++) {
                double
                        minx = transformedTriSet[i].transformedVector[0].x,
                        maxx = transformedTriSet[i].transformedVector[0].x,
                        miny = transformedTriSet[i].transformedVector[0].y,
                        maxy = transformedTriSet[i].transformedVector[0].y;
                for (int j = 1; j < 3; j++) {
                    minx = Math.min(minx, transformedTriSet[i].transformedVector[j].x);
                    maxx = Math.max(maxx, transformedTriSet[i].transformedVector[j].x);
                    miny = Math.min(miny, transformedTriSet[i].transformedVector[j].y);
                    maxy = Math.max(maxy, transformedTriSet[i].transformedVector[j].y);
                }
                ...
                DDALine((int) transformedTriSet[i].transformedVector[0].x, (int) transformedTriSet[i].transformedVector[0].y, transformedTriSet[i].triReference.vertex[0].color,
                        (int) transformedTriSet[i].transformedVector[1].x, (int) transformedTriSet[i].transformedVector[1].y, transformedTriSet[i].triReference.vertex[1].color);
                DDALine((int) transformedTriSet[i].transformedVector[0].x, (int) transformedTriSet[i].transformedVector[0].y, transformedTriSet[i].triReference.vertex[0].color,
                        (int) transformedTriSet[i].transformedVector[2].x, (int) transformedTriSet[i].transformedVector[2].y, transformedTriSet[i].triReference.vertex[2].color);
                DDALine((int) transformedTriSet[i].transformedVector[1].x, (int) transformedTriSet[i].transformedVector[1].y, transformedTriSet[i].triReference.vertex[1].color,
                        (int) transformedTriSet[i].transformedVector[2].x, (int) transformedTriSet[i].transformedVector[2].y, transformedTriSet[i].triReference.vertex[2].color);
            }
        }
        if ((camera.obMode & Camera.SHOW_TRIANGLE) != 0) {
            for (int i = 0; i < transformedTriSet.length; i++) {
                if (Vector.dotProduct(transformedTriSet[i].triReference.getNormalVector(), camera.lookDirection) < 0)
                    continue;
                double
                        minx = transformedTriSet[i].transformedVector[0].x,
                        maxx = transformedTriSet[i].transformedVector[0].x,
                ...
                for (int j = Math.max((int) minx, 0); j < Math.min((int) maxx, width); j++) {
                    for (int k = Math.max((int) miny, 0); k < Math.min((int) maxy, height); k++) {
                        int v = judge(transformedTriSet[i].transformedVector, j, k);
                        if (v == 1 || v == 0) {
                            double[] zres = interpolationZ(j, k, transformedTriSet[i]);
                            ...
                            if (bUnitSet[j][k].zValue >= zres[3]) {
                                ...
                                bUnitSet[j][k].transformedTri = transformedTriSet[i];
                            }
                        }
                    }
                }
